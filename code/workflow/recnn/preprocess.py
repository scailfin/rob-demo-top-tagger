...
def run(config_dir, static_data_dir, run_dir):
    ...
    tt_ref_tree_file = os.path.join(run_dir, 'tree_test_jets.pkl')
    logging.info('Loading toptag_reference_dataset {}'.format(tt_ref_tree_file))
    with open(tt_ref_tree_file, 'rb') as f:
        tt_ref_test_data = pickle.load(f,encoding='latin-1')
    # Preprocess
    tt_ref_x = list()
    for jet in np.asarray([x for (x,y) in tt_ref_test_data]):
        content = preprocess.rewrite_content(jet)
        samples = preprocess.extract_nyu_samples(pp.permute_by_pt(content))
        tt_ref_x.append(samples)
    tt_ref_y = np.asarray([y for (x,y) in tt_ref_test_data])
    # Load transformer
    transformer_file = os.path.join(static_data_dir, 'transformer.pkl')
    ...
    # Save trees
    output_file = os.path.join(run_dir, 'processed_test_jets.pkl')
